...
def perform_inference_image(image_path, model, colors, classes, draw_bbox, mrcnn_visualize):
...
def custom_visualize(test_image, model, colors, classes, draw_bbox , mrcnn_visualize):
    ...
    if mrcnn_visualize:
        ...
        visualize.display_instances(test_image, detections['rois'], detections['masks'], detections['class_ids'], classes,
                                detections['scores'])
...
#         output = custom_visualize(frame, model, colors, classes, draw_bbox, mrcnn_visualize)
...
if args['image']:
    my_config = MyConfig()
    my_config.display()
    colors, model, classes = prepare_mrcnn_model(args['model_path'], args['model_name'], args['class_names'], my_config)
...
#     my_config.display()
#     colors, model, classes = prepare_mrcnn_model(args['model_path'], args['model_name'], args['class_names'], my_config)
#     perform_inference_video(use_camera, video_path, model, colors, classes, args['draw_bbox'], args['mrcnn_visualize'])
